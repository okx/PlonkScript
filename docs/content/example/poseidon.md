...
fn full_round_state(a1, a2, a3, f1, f2, f3, idx) {
    let mds = global::mds;
    let next = [
        a1[1],
        a2[1],
    ...
    pow_5(a1 + f1) * mds[idx][0]
        + pow_5(a2 + f2) * mds[idx][1] 
        + pow_5(a3 + f3) * mds[idx][2]
        - next[idx];
}

gate full_round(a1, a2, a3, f1, f2, f3, s) {
    s <| full_round_state(a1, a2, a3, f1, f2, f3, 0);
    s <| full_round_state(a1, a2, a3, f1, f2, f3, 1);
    s <| full_round_state(a1, a2, a3, f1, f2, f3, 2);
}

// IN -> ARC -> M(.) -> CHECKPOINT <- ARC <- M(.) <- OUT
fn partial_round_linear(a1, a2, a3, a4, f2, f3, fb, idx) {
    let mds = global::mds;
    let mds_inv = global::mds_inv;
    let o1 = a1[1];
    let o2 = a2[1];
    let o3 = a3[1];
    a4 * mds[idx][0]
...
fn partial_round_sbox(a1, a2, a3, a4, f2, f3, f4) {
    let mds = global::mds;
    let mds_inv = global::mds_inv;
    let o1 = a1[1];
    let o2 = a2[1];
...
gate partial_round(a1, a2, a3, a4, f1, f2, f3, f4, f5, f6, s) {
    // state[0] round a
    s <| pow_5(a1 + f1) - a4;
    // state[0] round b
    s <| partial_round_sbox(a1, a2, a3, a4, f2, f3, f4);
    // state[1]
    s <| partial_round_linear(a1, a2, a3, a4, f2, f3, f5, 1);
    // state[2]
    s <| partial_round_linear(a1, a2, a3, a4, f2, f3, f6, 2);
}

gate pad_and_add(a1, a2, a3, s) {
    let init_s1 = a1[-1];
    let init_s2 = a2[-1];
    let init_s3 = a3[-1];
    let input1 = a1;
    ...
    let out_s2 = a2[1];
    let out_s3 = a3[1];
    s <| init_s1 + input1 - out_s1;
    s <| init_s2 + input2 - out_s2;
    s <| init_s3 - out_s3;
}

public selector s_full;
public selector s_partial;
public selector s_pad_and_add;
...
private advice a2;
private advice a3;
private advice a4; // mid partial round
...
fn gen_full_round(a1, a2, a3, f1, f2, f3, s, round, offset) {
    let round_constants = global::round_constants;
    let mds = global::mds;
...
fn gen_partial_round(a1, a2, a3, a4, f1, f2, f3, f4, f5, f6, s, round, offset) {
    let round_constants = global::round_constants;
    let mds = global::mds;
    f1[offset] <-- round_constants[round][0];
    f2[offset] <-- round_constants[round][1];
...
region permute_state(ins, a1, a2, a3, a4, f1, f2, f3, f4, f5, f6, s_full, s_partial, s_pad_and_add) {
    a1[0] <== ins[0];
    a2[0] <== ins[1];
    a3[0] <== ins[2];
    ...
    // full rounds
    for i in 0..rf {
        gen_full_round(a1, a2, a3, f1, f2, f3, s_full, i, i);
    }
    // partial rounds
    for i in 0..rp {
        gen_partial_round(a1, a2, a3, a4, f1, f2, f3, f4, f5, f6, s_partial, i * 2 + rf, i + rf);
    }
    // full rounds
    for i in 0..rf {
        gen_full_round(a1, a2, a3, f1, f2, f3, s_full, i + rf + rp * 2, i + rf + rp);
    }
...
let output = permute_state([in1, in2, pad], a1, a2, a3, a4, f1, f2, f3, f4, f5, f6, s_full, s_partial, s_pad_and_add);

out <== output;

full_round(a1, a2, a3, f1, f2, f3, s_full);
partial_round(a1, a2, a3, a4, f1, f2, f3, f4, f5, f6, s_partial);
```